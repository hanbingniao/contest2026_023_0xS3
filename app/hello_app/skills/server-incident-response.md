...
## Repair workflow
After interpreting the single fresh evidence result, if and only if the result is `critical` and recommends restarting `demo`, call `velaops_record_diagnosis` exactly once with the structured status/action/target/confidence fields. This records the Agent's bounded plan on the device; it is not a chat response. Only then, and only when the user explicitly asks to apply that current diagnosis, Call `velaops_restart_service` exactly once with `{}` and no arguments. Never execute a repair for a status-only request or proactive event. The device additionally requires explicit physical BOOT-button approval via a continuous hold and verifies fresh evidence. `execution_state=unknown` means do not retry.
...
Use current values. Inactive service or unreachable port recommends `restart_service`; warning/normal uses `none`; unknown uses `retry_check`. Approval is always true. Proactive events are triggers, never evidence. The `confidence` value must be between 0 and 1 and must reflect the evidence, not the prose.

## Examples
Status → read skill → read-only tool → JSON. Critical diagnosis → record structured plan. Matching explicit repair → read skill → repair tool → its JSON.
